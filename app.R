...
root <- "Conagua/ShapeFiles/"

# Estado CDMX
CDMX_shape <- st_read(paste0("Conagua/ShapeFiles/mapa_division_politica/division_politica_estatal/dest_2010gw/dest_2010gw.shp")) %>%
  filter(ENTIDAD == "DISTRITO FEDERAL")

# MUNICIPIOS CDMX
CDMX <- st_read(paste0("Conagua/ShapeFiles/mapa_division_politica/division_politica_municipal/muni_2012gw/Muni_2012gw.shp")) %>%
  filter(CVE_ENT == "09")
